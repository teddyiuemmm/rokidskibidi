...
save(Tabs.Logger:AddButton({
    Title = "Execute Once",
    Callback = function()
        fireSignal(signalType, id)
    end
}))

    save(Tabs.Logger:AddToggle("Auto" .. id, {
        Title = "Auto Execute ",
        Default = false,
        Callback = function(state)
